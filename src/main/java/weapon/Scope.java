...
public class Scope extends Attachment {
   ...
   * 
   * @param base
   * @throws AttachmentException if there are already 2 attachments on the base
   *                             Weapon
   */
  ...
  /**
   * Scopes increase the maxRange of the base Weapon, which may or may not be used
   * to calculate its damage Scopes also increase a weapon's power ScopeDamage =
   * BaseDamage*((maxRange - distance)/maxRange)
   * 
  ...
  public int fire(double distance) throws WeaponException {
    ...
    if (distance <= base.getMaxRange()) {
      ...
      scopeDamage = base.fire(distance)
          * (1 + ((double) getMaxRange() - distance) / (double) getMaxRange());
    } else if (distance <= this.getMaxRange()) {
      // if the distance is within the extended range
      // calculate the base weapon's damage at the base maxRange and add 5 to that
      // damage
      scopeDamage = base.fire(base.getMaxRange()) + 5;
    } else {
      // otherwise it's just too far away, call base.fire() should do 0 damage, but
      // update ammo and shots fired
      scopeDamage = base.fire(distance);
    }
